...
export class SecuriaClientService {
  ...
  static async createClient(clientData: any, userId: string) {
    // Always set the root email from applicant.email
    if (clientData.applicant && clientData.applicant.email) {
      clientData.email = clientData.applicant.email;
    }
    // Log the clientData before saving
    console.log('Creating SecuriaClient with data:', JSON.stringify(clientData, null, 2));
